...
export default function MovieCard({ movie, onDelete }: MovieCardProps) {
  ...
  return (
    <div className="group relative h-[420px] bg-white/5 border border-white/10 rounded-xl overflow-hidden hover:border-white/20 transition-colors flex flex-col">
      {/* Poster - fixed height */}
      ...
      {/* Content - fixed height with overflow hidden */}
      <div className="flex-1 p-3 flex flex-col overflow-hidden">
        ...
        {movie.plot && movie.plot !== 'N/A' && (
          <p className="text-xs text-white/50 line-clamp-3 leading-relaxed flex-1">
            {movie.plot}
          </p>
        )}
        ...
        {movie.director && movie.director !== 'N/A' && (
          <div className="flex items-center gap-1.5 text-[10px] text-white/40 pt-2 mt-auto border-t border-white/10">
            <User size={10} />
